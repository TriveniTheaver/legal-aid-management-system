  ...
  try {
    ...
    if (userType === 'client' || userType === 'verified_client') {
      caseData = await Case.findOne({ _id: caseId, user: userId })
        ...
        .populate('currentLawyer', 'fullName email phoneNumber');
...
router.post("/emergency-fix-lawyer/:caseId", protect, async (req, res) => {
  try {
    ...
    const assignment = await CaseLawyerAssignment.findOne({ case: caseId }).sort({ assignedAt: -1 });
...
router.get("/debug/lawyer-assignments/:caseId", protect, async (req, res) => {
  try {
    ...
    const assignments = await CaseLawyerAssignment.find({ case: caseId })
...
router.post("/:caseId/request-filing", protect, async (req, res) => {
  try {
    ...
    console.log(`📋 Client ${userId} requesting court filing for case ${caseId}`);
    
    // Use the new case flow service
    const CaseFlowService = require('../services/caseFlowService');
    const result = await CaseFlowService.requestCourtFiling(caseId, userId, message);
    ...
    console.log(`✅ Court filing requested successfully for case ${caseId}`);
...
router.post("/fix-stolen-cases", protect, async (req, res) => {
  try {
    ...
    for (const caseItem of allCases) {
      // Get all assignments for this case
      const assignments = await CaseLawyerAssignment.find({ case: caseItem._id })
        .populate('lawyer', 'fullName name email')
        .sort({ createdAt: -1 });
