...
const scheduleCase = async (req, res) => {
  try {
    ...
    // Validate that the hearing date is not in the past
    const hearingDateTime = new Date(hearingDate + 'T00:00:00');
    ...
    if (hearingDateTime < today) {
      return res.status(400).json({
        success: false,
        message: 'Cannot schedule hearings on past dates. Please select a future date.'
      });
    }
    ...
    await CourtScheduleRequest.findByIdAndUpdate(requestId, {
      isScheduled: true,
      scheduledDate: new Date(hearingDate + 'T00:00:00'),
    ...
    // Ensure we have a lawyer assigned before updating status
    if (!currentCase.currentLawyer) {
      console.log('⚠️ WARNING: No currentLawyer found, attempting to restore from assignment...');
      ...
      try {
        ...
        if (assignment) {
          currentCase.currentLawyer = assignment.lawyer;
          console.log(`✅ Restored currentLawyer from assignment: ${assignment.lawyer}`);
        } else {
          console.log('❌ No valid assignment found, using schedule request lawyer');
          currentCase.currentLawyer = scheduleRequest.lawyer;
        }
      } catch (error) {
    ...
    // Update assignment status to active if it was accepted
    if (currentCase.currentLawyer) {
      try {
        const CaseLawyerAssignment = require('../Model/CaseLawyerAssignment');
        const assignment = await CaseLawyerAssignment.findOne({
          case: scheduleRequest.case,
          lawyer: currentCase.currentLawyer
        });
        
        if (assignment && assignment.status === 'accepted') {
          assignment.status = 'active';
          assignment.activatedAt = new Date();
          assignment.caseStatusWhenActivated = 'hearing_scheduled';
          await assignment.save();
          console.log(`✅ Updated assignment status to active for scheduled case ${scheduleRequest.caseNumber}`);
        }
      } catch (error) {
