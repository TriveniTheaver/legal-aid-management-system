...
const acceptAdjournmentRequest = async (req, res) => {
  try {
    ...
    const oldHearingTime = caseItem.hearingTime;
    
    caseItem.hearingDate = new Date(newHearingDate + 'T00:00:00');
    ...
    if (scheduledCase) {
      ...
      console.log(`🔄 ADJOURNMENT: Rescheduling case ${caseItem.caseNumber}`);
      console.log(`  Old date: ${oldHearingDate ? oldHearingDate.toLocaleDateString('en-LK') : 'None'}`);
      console.log(`  Old time: ${oldHearingTime ? `${oldHearingTime.startTime}-${oldHearingTime.endTime}` : 'None'}`);
      console.log(`  New date: ${new Date(newHearingDate + 'T00:00:00').toLocaleDateString('en-LK')}`);
      console.log(`  New time: ${newHearingTime ? `${newHearingTime.startTime}-${newHearingTime.endTime}` : 'None'}`);
      
      scheduledCase.hearingDate = new Date(newHearingDate + 'T00:00:00');
      ...
      scheduledCase.adjournmentNotes = `Rescheduled from ${oldHearingDate ? oldHearingDate.toLocaleDateString('en-LK') : 'original date'} - ${schedulerNotes || 'No notes provided'}`;
      await scheduledCase.save();
      console.log(`✅ ScheduledCase updated for case ${caseItem.caseNumber} - New date: ${new Date(newHearingDate + 'T00:00:00').toLocaleDateString('en-LK')}`);
    } else {
    ...
    // For now, we'll skip notification creation to prevent errors
    // TODO: Implement proper notification system for adjournment requests
    console.log(`Adjournment request accepted for case ${caseItem.caseNumber} - New date: ${new Date(newHearingDate + 'T00:00:00').toLocaleDateString('en-LK')}`);
