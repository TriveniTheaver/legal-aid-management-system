...
const { validate, cardValidationSchema, userRegistrationSchema, validateCardExpiry } = require('../Middleware/validation');
...
const registerUnverifiedClient = async (req, res) => {
    try {
        ...
        // Validate NIC
        if (nic.length === 12) {
            // 12-character format: first 4 digits should be year (2000-2007)
            ...
            if (!/^\d{12}$/.test(nic)) {
                ...
                return res.status(400).json({ 
                    message: "Age is not sufficient to register. Person must be at least 16 years old." 
                });
            }
        } else if (nic.length === 10) {
            // 10-character format: first 9 digits, last character alphabetic
            if (!/^\d{9}[a-zA-Z]$/.test(nic)) {
                return res.status(400).json({ 
                    message: "NIC must be 9 digits followed by one letter" 
                });
            }
        } else {
            return res.status(400).json({ 
                message: "NIC must be either 10 or 12 characters" 
            });
        }
        ...
        if (!/^\+94\d{9}$/.test(phoneNumber)) {
            return res.status(400).json({ 
                message: "Phone number must be +94 followed by exactly 9 digits" 
            });
        }
...
const registerUnverifiedLawyer = async (req, res) => {
    try {
        ...
        // Validate required fields
        if (!fullName || !nic || !email || !password || !phoneNumber || !address || !lawyerType || !passoutYear) {
            return res.status(400).json({ 
                message: "All fields are required" 
            });
        }
