...
const { validateCardExpiry } = require('../Middleware/validation');
  ...
  try {
    ...
    if (!/^\d{2}\/\d{2}$/.test(paymentDetails.expiryDate)) {
      ...
      if (monthNum < 1 || monthNum > 12) {
        ...
        validationErrors.expiryDate = 'Year must be 2025 or later';
      } else {
    ...
    const billingValidationErrors = {};
    
    // Validate name (letters and spaces only)
    if (!/^[a-zA-Z\s]+$/.test(billingInfo.fullName.trim())) {
      billingValidationErrors.fullName = 'Name should contain only letters and spaces';
    }
    
    // Validate email format
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(billingInfo.email.trim())) {
      billingValidationErrors.email = 'Please enter a valid email address';
    }
...
const processIndividualServicePayment = async (req, res) => {
  try {
    ...
    // Validate payment details with comprehensive card validation
    if (!paymentDetails || !paymentDetails.cardNumber || !paymentDetails.expiryDate || !paymentDetails.cvv) {
      return res.status(400).json({
        success: false,
        message: 'Complete payment details are required (card number, expiry date, CVV)'
      });
    }
